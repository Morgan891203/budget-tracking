...
import type { Prisma, Expense, Debt } from '@prisma/client';
import { PiggyBank, CreditCard, FileMinus, Heart } from 'lucide-react';
...
export default async function DashboardPage({
  ...
  searchParams: { summaryPage?: string; expensesPage?: string; deductionsPage?: string };
}) {
  const ITEMS_PER_PAGE = 5;

  const summaryPage = parseInt(searchParams.summaryPage ?? '1', 10);
  const expensesPage = parseInt(searchParams.expensesPage ?? '1', 10);
  const deductionsPage = parseInt(searchParams.deductionsPage ?? '1', 10);
  ...
  const momDebts: Debt[] = await prisma.debt.findMany({
    where: { person: 'Mom' },
    orderBy: {
      date: 'desc',
    },
  ...
  const monthlySummary: Record<string, { gross: number; deductions: number }> = {};
  ...
  const monthlyExpenseSummary: Record<string, { total: number }> = {};
  ...
  const monthlyMomDebtSummary: Record<string, { balance: number }> = {};
  ...
  const monthlyDeductionDetails: Record<
    string,
  ...
  // Pagination for expenseSummaryList
  const totalExpenseSummaryPages = Math.ceil(
    expenseSummaryList.length / ITEMS_PER_PAGE
  );
  ...
  // Pagination for deductionDetailsList
  const totalDeductionPages = Math.ceil(
    deductionDetailsList.length / ITEMS_PER_PAGE
  );
        ...
        <div className="bg-white rounded-lg shadow-md">
          <div className="flex justify-between items-center gap-4 p-6 border-b border-gray-200">
            ...
            <div className="text-right">
              <p className={`font-semibold text-2xl ${totalNetBalance >= 0 ? 'text-indigo-600' : 'text-red-600'}`}>
          ...
          <ul className="divide-y divide-gray-200">
            {paginatedSummaryList.length === 0 ? (
              ...
              paginatedSummaryList.map((summary) => (
                <li key={summary.date.toISOString()} className="p-6 flex justify-between items-center">
                  <div className="flex-grow">
                    <p className="font-semibold text-gray-800 text-lg">{format(summary.date, 'MMMM yyyy')}</p>
            ...
            <div className="p-6 text-center">
              ...
              <p
                className={`text-3xl font-bold ${
                  overallMomDebtBalance >= 0 ? 'text-green-600' : 'text-red-600'
                }`}
              >
              ...
              {paginatedExpenseSummaryList.length === 0 ? (
                ...
                paginatedExpenseSummaryList.map((summary) => (
                  <li key={summary.date.toISOString()}>
                    <Link
                      href={`/expense?year=${format(summary.date, 'yyyy')}&month=${format(summary.date, 'MM')}`}
            ...
            <ul className="divide-y divide-gray-200">
              {paginatedDeductionDetailsList.length === 0 ? (
                ...
                paginatedDeductionDetailsList.map((summary) => (
                  <li key={summary.date.toISOString()}>
                    <Link
                      href={`/salary?year=${format(summary.date, 'yyyy')}&month=${format(summary.date, 'MM')}`}
                    ...
                    >
                      ...
                      <ul className="mt-2 space-y-1 text-sm text-gray-600">
                        {Object.entries(summary.breakdown).map(([category, amount]) => (
